...
bank_db = client['bank_details']
loan_db = client['LoanML']
bank_collection = bank_db['banks']
admin_db = client['admin_details']
...
def register_bank():
    ...
    try:
        bank_collection.insert_one(data)
...
def login_bank():
    ...
    bank = bank_collection.find_one({'loginUsername': username, 'loginPassword': password})
    if bank:
